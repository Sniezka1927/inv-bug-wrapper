...
test_contract.rs: test_contract
	cd ink-wrapper && \
		...
		| rustfmt --edition 2021 > ../src/test_contract.rs


.PHONY: generate-wrappers
generate-wrappers: test_contract.rs # Generate wrappers for test contracts.
...
# 	cd ../ && cargo clippy --all-features -- --no-deps -D warnings
